function theData = MMT_fMRI_test(thePath,listName,startTrial)
...
sTestNum = input('Enter test number: \n [1-3] scanned word blocks, \n [4] non-scanned word block, \n [5-6] scanned (?) picture blocks: ');
...
stimRect1 = round([Rect(3)/2-rectWidth(1)/2, Rect(4)/2-rectHeight(1)/2,Rect(3)/2+rectWidth(1)/2, Rect(4)/2+rectHeight(1)/2]); 
...
% stimRect1 = round([Rect(3)/2-(rectWidth(1)+75), Rect(4)/2-(rectHeight(1)+75),Rect(3)/2-75, Rect(4)/2-75]); 
% 
...
% stimRect2 = round([Rect(3)/2+75, Rect(4)/2-(rectHeight(1)+75), Rect(3)/2+(rectWidth(1)+75), Rect(4)/2-75]);
% 
...
% stimRect3 = round([Rect(3)/2+75, Rect(4)/2++75, Rect(3)/2+(rectWidth(1)+75), Rect(4)/2+(rectHeight(1)+75)]);        %lower right of fixation
% 
...
% stimRect4 = round([Rect(3)/2-(rectWidth(1)+75), Rect(4)/2+75, Rect(3)/2-75, Rect(4)/2+(rectHeight(1)+75)]);        %lower left of fixation
...
stimTime = 2.0;         % stim duration
ITI = 2.0;

% ITI = 2.0;            % for behavioral piloting
...
% STIM ...
     ...
     % DETERMINE WHETHER TESTING WORD (TARGETS) OR PICS (DISTRACTORS) based on experimenter's input
     
     if sTestNum == 1 || sTestNum == 2  %WORD BLOCK, SO DRAW WORD
         DrawFormattedText(Window, word{trial},'center','center',255);           %white text
     else  %PIC BLOCK, SO DRAW PIC
...
fprintf(fid, ('subjNum\tsessInput\tstudySess\tonset\tword\twordColor\tdistCond\tdist\tdistPos\tdistCat\tOldNew\tresp\tRT\n'));
for n = 1:trialcount
    fprintf(fid, '%f\t%f\t%f\t%f\t%s\t%f\t%s\t%s\t%f\t%s\t%s\t%s\t%f\n',...
        theData.sNum,theData.sTestNum,theData.stsess(n),theData.onset(n),theData.word{n},theData.wordColor(n),...
        theData.distCond{n}, theData.dist{n},theData.distPos(n), theData.distCat{n}, theData.oldNew{n}, ...
